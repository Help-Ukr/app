...
export const FieldImage: FC<{ formField: MobXForm.InputProps<ImageType | undefined> }> = observer(({ formField }) => {
        ...
        <ImageUploading value={value} onChange={handleChange}>
            {({ onImageUpload, onImageRemove }) => (
                <Box sx={{ position: 'relative', ...sx100 }}>
                    ...
                    {src && (
                        <IconButton onClick={() => onImageRemove(0)} sx={{ position: 'absolute', top: 2, right: 2 }}>
                            <HighlightOffIcon sx={{ width: 24, height: 24 }} color="inherit" enableBackground="red" />
                        </IconButton>
                    )}
                </Box>
            )}
